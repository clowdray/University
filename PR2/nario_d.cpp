...
void testValues3(escenari &e);
void bridgeGenerator(escenari &e);
void showGraph(escenari e);
void optimumPath(escenari e, int ini, int fin, bool &trobat, Stack &s, bool &solucio, Stack &a);
...
void copyStack(Stack s, Stack &a);
void pause();
...
void testValues3(escenari &e) {
    /**
    * Què fa: generació hardcodejada per els test 3 i 4
    * Entrades: varibale e de tipus escenari
    * Sortides: #
    */
    e.illes[0].numIlla = 32;
    e.illes[1].numIlla = 40;
    e.illes[2].numIlla = 55;
    e.illes[3].numIlla = 62;
    e.illes[4].numIlla = 74;
    e.illes[5].numIlla = 76;
    e.illes[6].numIlla = 78;
    e.illes[7].numIlla = 79;
    ...
    */
    ...
    if (ini == fin) {
        ...
        while (i<joc && !trobat){
            if (e.matriu[k][i] == 1) {
                if (e.illes[i].numIlla == fin){
                    ...
                    int enemicsActual = enemics(s, e);
                    int enemicsGuardat = enemics(a, e);
                    if (enemicsActual < enemicsGuardat){
                        copyStack(s,a);
                    }
                    trobat = false;
                }
        ...
        while (d.top > -1 && !trobat) {
            k = treure(d);
            optimumPath(e, k, fin, trobat, s, solucio, a);
        }
        if (!trobat) {
            trash = treure(s);
            k = trash;
...
void copyStack(Stack s, Stack &a) {
    /**
    * Què fa: copia tota la informació de un stack a un altre
    * Entrades: el stack a copiar, el stack amb les dades del anterior
    * Sortides: #
    */
    int i = 0;
    while (i<s.top+1){
        ...
        switch(opcio){
            ...
            case 2:
                if (e.illes[0].numIlla < 11 || e.illes[0].numIlla > 99){
					...
					ini = 32;
					fin = 62;
					optimumPath(e, ini, fin, trobat, s, solucio, a);
					if (!solucio) {
						cout << "No existeix camí possible" << endl;
						cout << "En el trajecte el Mario s'ha trobat 0 enemics" << endl;
					}
					else {
						cout << endl << flush(a) << fin << endl;
						cout << "En el trajecte el Mario s'ha trobat " << enemics(a, e) << " enemics" << endl;
					}
				}
            break;
            case 3:
