...
void pathFinder(escenari e, int ini, int fin, bool &trobat, Stack &s);
...
void newGraph(escenari &e) {
    /**
    * Què fa: Generem les dades de un nou escenari, donant valor al num de la illa, els
              seus enemics, així com la matriu d'ajacència
    * Entrades: variable e buida per ser emplenada
    * Sortides: variable e modificada
    */
    ...
    for (int i=0;i<joc;i++) {
        do {
            numIlla = rand() % 88 + 11; // random de 11 a 99
            if (!existIsland(e, numIlla)){
                repetit = false;
                e.illes[i].numIlla = numIlla;
            }
            else {
                repetit = true;
            }
        } while (repetit);
        e.illes[i].numEnemics = rand() % 10; // random de 0 a 9
    }
    bridgeGenerator(e);
}
...
void pathFinder(escenari e, int ini, int fin, bool &trobat, Stack &s) {
    /**
    * Què fa: Buscar el cami més curt i mostrar, el camí recorregut i passar el nombre de enemics trobats
        ...
        switch(opcio){
            ...
            case 2:
				if (e.illes[0].numIlla < 11 || e.illes[0].numIlla > 99){
					...
					cout << "Introdueixi la illa inicial: ";
					cin >> ini;
					cout << "Introdueixi la illa final: ";
					cin >> fin;
					pathFinder(e, ini, fin, trobat, s);
					if (!trobat){
